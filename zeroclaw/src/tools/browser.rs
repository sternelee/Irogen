...
//! `--features browser` and selected through config.
...
impl BrowserTool {
    ...
    async fn resolve_backend(&self) -> anyhow::Result<ResolvedBackend> {
        ...
        match configured {
            ...
            BrowserBackendKind::Auto => {
                ...
                if let Some(err) = computer_use_err {
                    anyhow::bail!(
                        "browser.backend='auto' needs agent-browser CLI, browser, or valid computer-use sidecar (error: {err})"
                    );
                }

                anyhow::bail!(
                    "browser.backend='auto' needs agent-browser CLI, browser, or computer-use sidecar"
                )
            }
        }
    }
    ...
    async fn execute_rust_native_action(
        ...
        #[cfg(not(feature = "browser"))]
        {
            let _ = action;
            anyhow::bail!(
                "Rust-native browser backend is not compiled. Rebuild with --features browser"
            )
        }
    }
