...
impl OptimizedMessageRouter {
    ...
    pub async fn route_messages_batch(&self, messages: Vec<Message>) -> Vec<Result<Option<Message>>> {
        ...
        // 更新统计信息
        self.stats.pending_messages.fetch_add(batch_size, Ordering::Relaxed);
        self.stats.total_messages.fetch_add(batch_size as u64, Ordering::Relaxed);
        ...
        // 更新统计信息
        self.stats.pending_messages.fetch_sub(batch_size, Ordering::Relaxed);
        self.stats.successful_messages.fetch_add(successful_count, Ordering::Relaxed);
        self.stats.failed_messages.fetch_add(failed_count, Ordering::Relaxed);

        let processing_time = start_time.elapsed().as_micros() as u64;
        self.stats.avg_processing_time_us.store(processing_time, Ordering::Relaxed);
...
impl MessageRetryManager {
    ...
    pub async fn add_retry_message(&self, message: Message, error: String) {
        ...
        self.retry_stats.total_retries.fetch_add(1, Ordering::Relaxed);
    }
            ...
            loop {
                ...
                for mut retryable in ready_messages {
                    ...
                    for result in results {
                        if result.is_ok() {
                            success = true;
                            retry_stats.successful_retries.fetch_add(1, Ordering::Relaxed);
                            info!(
                    ...
                    if !success {
                        ...
                        let delay_ms = (retry_interval_ms as f64
                            * backoff_factor.powi(retryable.retry_count as i32)) as u64;
    ...
    pub async fn start_monitoring(
        ...
        tokio::spawn(async move {
            loop {
                ...
                metrics.avg_response_time_us.store(avg_time, Ordering::Relaxed);
                metrics.error_rate_percent.store(error_rate, Ordering::Relaxed);
...
mod tests {
    ...
    async fn test_retry_manager() {
        ...
        retry_manager.add_retry_message(message.clone(), "test error".to_string()).await;
    ...
    async fn test_performance_monitor() {
        ...
        let _handle = monitor.start_monitoring(router.clone(), event_manager.clone()).await;
