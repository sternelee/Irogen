...
impl OptimizedMessageHandler for MessageHandlerAdapter {
    async fn handle_message_optimized(&self, message: &Message) -> Result<Option<Message>, anyhow::Error> {
        let start_time = Instant::now();
        self.stats.total_processed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        ...
        match &result {
            Ok(_) => {
                self.stats.successful_processed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            }
            Err(e) => {
                self.stats.failed_processed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                warn!("Handler {} failed to process message {}: {}", self.name, message.id, e);
            }
        }

        // 更新平均处理时间（简单的移动平均）
        let current_avg = self.stats.avg_processing_time_us.load(std::sync::atomic::Ordering::Relaxed);
        let new_avg = (current_avg + processing_time) / 2;
        self.stats.avg_processing_time_us.store(new_avg, std::sync::atomic::Ordering::Relaxed);
...
impl OptimizedCommunicationManager {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        // 启动重试处理器
        let retry_handle = self.retry_manager
            .start_retry_processor(self.optimized_router.clone())
            .await;

        // 启动性能监控
        let monitor_handle = self.performance_monitor
            .start_monitoring(
    ...
    pub async fn register_message_handler(&self, handler: Arc<dyn MessageHandler>) {
        let name = format!("adapter_{}", uuid::Uuid::new_v4().to_string()[..8].to_string());
        let adapter = Arc::new(MessageHandlerAdapter::new(handler, name.clone()));

        // 注册到优化路由器
        self.optimized_router.register_handler(adapter.clone()).await;
    ...
    pub async fn register_event_listener(&self, listener: Arc<dyn EventListener>) {
        let name = format!("event_adapter_{}", uuid::Uuid::new_v4().to_string()[..8].to_string());
        let adapter = Arc::new(EventListenerAdapter::new(listener, name.clone()));

        // 注册到优化事件管理器
        self.optimized_event_manager.register_listener(adapter.clone()).await;
    ...
    pub async fn send_message(&self, message: Message) -> Result<()> {
        ...
        if !has_success {
            ...
            for result in results {
                if let Err(e) = result {
                    self.retry_manager.add_retry_message(message.clone(), e.to_string()).await;
                }
            }
        }
    ...
    pub async fn receive_incoming_message(&self, message: Message) -> Result<()> {
        ...
        if failed_count == results.len() && failed_count > 0 {
            self.retry_manager.add_retry_message(
                message.clone(),
                "All optimized handlers failed".to_string(),
            ).await;
        }
...
mod tests {
    use super::*;
    use crate::event_manager::{TerminalEventListener, EventType};
    ...
    async fn test_message_handler_adapter() {
        ...
        assert_eq!(stats.total_processed.load(std::sync::atomic::Ordering::Relaxed), 1);
        assert_eq!(stats.successful_processed.load(std::sync::atomic::Ordering::Relaxed), 1);
    }

    #[tokio::test]
    async fn test_event_listener_adapter() {
        let original = Arc::new(TerminalEventListener::new("test".to_string()));
        let adapter = Arc::new(EventListenerAdapter::new(original, "test_adapter".to_string()));
    ...
    async fn test_optimized_communication_manager() {
        ...
        assert!(metrics.messages_per_second.load(std::sync::atomic::Ordering::Relaxed) >= 0);
        assert!(router_stats.total_messages.load(std::sync::atomic::Ordering::Relaxed) >= 0);
        assert!(event_stats.total_events.load(std::sync::atomic::Ordering::Relaxed) >= 0);
    }
}