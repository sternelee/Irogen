...
interface ChatViewProps {
  ...
  onPermissionResponse: (requestId: string, approved: boolean, reason?: string) => Promise<void>
...
export function ChatView(props: ChatViewProps) {
  ...
  return (
    <div class="flex flex-col h-full">
          ...
          <Show when={props.thinking && !props.isStreaming}>
            <div class="flex items-start gap-3 py-4">
              <div class="w-8 h-8 rounded-lg bg-cyan-500/10 flex items-center justify-center flex-shrink-0">
                <Loader class="w-4 h-4 text-cyan-400 animate-spin" />
              </div>
              <div class="flex items-center gap-2 text-slate-400">
                <span>Thinking</span>
                <span class="flex gap-1">
                  <span class="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-bounce" style={{ 'animation-delay': '0ms' }} />
                  <span class="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-bounce" style={{ 'animation-delay': '150ms' }} />
                  <span class="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-bounce" style={{ 'animation-delay': '300ms' }} />
                </span>
              </div>
            </div>
          </Show>
      ...
      {/* Input Area */}
      <div class="flex-shrink-0 border-t border-slate-700 bg-slate-800/50">
